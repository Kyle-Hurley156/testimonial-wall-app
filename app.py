...
# This logic uses the live database URL on Render, but falls back to a local file if it's not set.
DATABASE_URL = os.getenv('DATABASE_URL')
if DATABASE_URL and DATABASE_URL.startswith("postgres://"):
    DATABASE_URL = DATABASE_URL.replace("postgres://", "postgresql://", 1)
app.config['SQLALCHEMY_DATABASE_URI'] = DATABASE_URL or 'sqlite:///' + os.path.join(basedir, 'testimonials.db')
...
app.config['ADMIN_KEY'] = os.getenv('ADMIN_KEY', 'default-admin-key')
...
class User(UserMixin, db.Model):
    ...
    wall_title = db.Column(db.String(100), default="What Our Customers Say")
...
def signup():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            flash(f'Error creating customer account: {e}', 'error')
            return redirect(url_for('signup'))
...
@app.route('/collect/<int:user_id>', methods=['POST'])
...
@login_required
def success():
    # Note: We use webhooks to update the subscription status in a real app
    # For now, we'll just update it on success for simplicity.
...
@app.route('/admin/<string:admin_key>', methods=['GET', 'POST']) # <-- THIS IS THE FIX
def admin(admin_key):
    if admin_key != app.config['ADMIN_KEY']:
        return "Unauthorized", 403
    
    if request.method == 'POST':
        new_code_str = 'SSR-' + secrets.token_hex(4).upper()
